...
//  BasicFramework
//
//  Created by Rainy on 2016/11/11.
//  Copyright © 2016年 Rainy. All rights reserved.
//

#ifndef ServerLoadControl_h
#define ServerLoadControl_h
...
/*
 ...
 //观察者回调-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
 ...
 当 self.player.status ==  AVPlayerStatusReadyToPlay时，音乐就会开始正常播放，另外两种状态音乐是无法播放的，可以在上面方法相应状态里给出提示。这里需要特别强调一点的是观察者监听的对象是self.player.currentItem，而不是self.player，而当监听的属性发生改变时，观察者回调的方法里需要查看的是self.player.status。当然，你也可以不这么干，但是我尝试过好几次，不这么干的后果是无法监听到self.player.status属性的改变。
...
//更新缓冲进度条
 ...
 /**
 监听音乐播放进度 @param interval 监听的时间间隔，用来设置多长时间回调一次 @param queue    队列，一般传主队列 @param block    回调的block，会把当前的播放时间传递过来 @return 监听的对象
...
//移动滑块调整播放进度- (IBAction)playSliderValueChange:(UISlider *)sender
{    //根据值计算时间
...
//获取音频会话
AVAudioSession *session = [AVAudioSession sharedInstance];    //设置类型是播放。
[session setCategory:AVAudioSessionCategoryPlayback error:nil];    //激活音频会话。
...
//音乐锁屏信息展示- (void)setupLockScreenInfo
{    
 // 1.获取锁屏中心
